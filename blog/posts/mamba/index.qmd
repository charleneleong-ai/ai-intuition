---
title: "Structured State Space Sequence Models (S4) Explained: Mamba"
...
The Mamba model architecture has recently emerged as a promising alternative to deep sequence modeling overcoming key limitations in other model families such as transformers, recurrent neural networks (RNNs), convolutional neural networks (CNNs), particularly for modelling long-range dependencies (LRDs).

They come from the family of structured state space models (SSMs), particularly useful for modelling long-range dependencies in continuous data such as time series and signal data [@tay2020long]. It offers a robust framework for handling long-range dependencies efficiently with up to 5x higher throughput than transformers, linear scaling with sequence length, with in-context learning performance improvement on real data up to million-length sequences [@gu2023mamba]. Therefore, their strengths are suited to problems in the long-range high frame-rate domain such as medical signal, speech, video or energy waveforms [@stanfordmedaialbertgus4].
...
::: {#fig-llm-long-context layout-ncol=2}
![OpenAI's GPT-4-128K](./gpt4_long_context.png){width=100% .lightbox}

![Anthropic's Claude 2.1](./claude_long_context.png){width=100% .lightbox}
...
|--------------------------------|--------------------------------------|-----------------------------------------|-----------------------------------------------------|
| **Training Time Complexity**   | $O(T \cdot d^2)$                     | $O(T^2 \cdot d)$                        | $O(T \cdot d)$                                      |
| **Training Space Complexity**  | $O(T \cdot d)$                       | $O(T^2 \cdot d)$                        | $O(T \cdot d)$                                      |
| **Inference Time Complexity**  | $O(T \cdot d^2)$                     | $O(T^2 \cdot d)$                        | $O(T \cdot d)$                                      |
| **Inference Space Complexity** | $O(d)$                               | $O(T^2 \cdot d)$                        | $O(d)$                                              |



## What are Structured State Space Sequence Models (S4)?
...
:::
To acommodate for parallelised training, we can unroll the linear recurrent form to yield a global convolutional representation to [@eq-s-conv]
...
![Visualising S4 vs S4D Results](./s4_vs_s4d.png){width=100% fig-align="center" .lightbox}
...
There are two core improvements that have now made Mamba perform competitively with transformers. The first

...
Big T is mounted with both exterioceptive sensors such as a mouse sensor, LIDAR and Intel RealSense and proprioceptive sensors such as IMU and encoders to capture odometry signals to inform the control algorithm. This project focuses on the control node to update the robot's motor direction and speed  from incoming motor and sensor inputs.
...
In order to convert the sensor data to publish the motor control signal, we first listen for robot's velocity commands `cmd_vel` and IMU data to calculate the wheel speeds and directions for each motor based on the robot's kinematics. The calculated angular velocities are then converted to PWM signals and published as a `geometry_msgs::Twist` message, where the linear fields represent PWM values and the angular fields represent motor directions.
...
We include the following headers
   ...
   - [`sensor_msgs/Imu.h`](http://docs.ros.org/en/kinetic/api/sensor_msgs/html/msg/Imu.html) to collect sensor data message type from the IMU. The IMU node [`razor_imu_9dof`](http://wiki.ros.org/razor_imu_9do) publishes messages to the `"imu"` topic in the ROS system to be used by motion and planning algorithms such as this one.
...
::: {add-from=control.cpp start-line=72 end-line=90 code-line-numbers="true"}
```{.cpp}
```
:::
...
::: {add-from=control.cpp start-line=41 end-line=69 code-line-numbers="true"}
```{.cpp}
```
:::
...
::: {add-from=control.cpp start-line=27 end-line=27 code-line-numbers="true"}
```{.cpp}
```
:::
...
::: {add-from=control.cpp start-line=21 end-line=31 code-line-numbers="true"}
```{.cpp}
```
:::
...
::: {add-from=control.cpp start-line=32 end-line=34 code-line-numbers="true"}
```{.cpp}
```
:::
